...
# define the file original paths to remove from xliff files after export
# These are phantom sections generated by xcodebuild -exportLocalizations
# that duplicate strings already present in another section
# Spent many hours trying to fix the Xcode settings, and settled on this
FILE_ORIGINAL_PATHS_TO_REMOVE=( \
    "OmnipodKit/Localizable.xcstrings" \
)
